...
- `nvpn wg-upstream-test --config-file <path>` is the safe-by-construction entry point: parses a wg-quick / Mullvad / Proton config, runs the userspace state machine against the upstream, reports whether the WG handshake completes within the timeout. **Does not create a tun device, does not modify routes** — running it can never blackhole the host's internet, even if the config is broken or the server is unreachable. Intended as the first integration test before any platform-routing follow-up wires the userspace runtime into the daemon's reconcile loop on macOS.
- `scripts/e2e-wireguard-exit-userspace-docker.sh` (`just e2e-wireguard-exit-userspace`) drives the userspace path against a real wg-quick server inside Docker; passes locally on macOS today against `wg show`-confirmed peer.
